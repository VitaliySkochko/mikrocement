      ...
      <div className="gallery-viewport" aria-live="polite">
        <div className="gallery-track" style={{ transform: `translate3d(-${activeIndex * 100}%, 0, 0)` }}>
          {items.map((item, index) => (
            <figure className="gallery-slide" key={item.image} aria-hidden={index !== activeIndex}>
        ...
        <button type="button" className="gallery-control gallery-control-prev" onClick={handlePrev} aria-label="Previous image">
          <span>‹</span>
        </button>
        <button type="button" className="gallery-control gallery-control-next" onClick={handleNext} aria-label="Next image">
          <span>›</span>
        </button>
      </div>
